...
@section('title', 'Detail Data Alat Berat - PT. Jambi Bara Sejahtera')
@section('content')
    <div class="container-fluid text-start">
        <h3 class="text-dark text-center mt-4">Detail Data Alat Berat</h3>
        <div class="row justify-content-center mt-4">
            <div class="col-11 col-sm-10 col-md-8 col-lg-7 col-xl-6">
                <div class="card clean-card">
                    <div class="card-img-top-container">
                        @if ($dataAlat->image)
                            <a href="{{ asset('storage/potoAlat/' . $dataAlat->image) }}" target="_blank">
                                <img class="card-img-top w-100 d-block"
                                    src="{{ asset('storage/potoAlat/' . $dataAlat->image) }}" alt="{{ $dataAlat->name }}"
                                    style="max-height: 400px; object-fit: cover;" />
                            </a>
                        @else
                            <img class="card-img-top w-100 d-block" src="{{ asset('storage/Image_not_available.png') }}"
                                alt="Image not available" style="max-height: 400px; object-fit: cover;" />
                        @endif
                    </div>
                        ...
                        <div class="container">
                            <div class="row d-flex justify-content-center">
                                <div class="row d-flex justify-content-center gap-2">
                                    ...
                                    <div class="row">
                                        <form class="p-0" action="{{ route('dataalat.destroy', $dataAlat) }}"
                                            method="post" id="deleteForm-{{ $dataAlat->id }}">
                                            @method('DELETE')
                                            @csrf
                                            <button type="button" class="btn btn-danger btn-block" style="width: 100%;"
                                                onclick="confirmDelete({{ $dataAlat->id }})">Hapus</button>
                                        </form>
                                    </div>
                                    <div class="row">
